...
class DataManager:
    ...
    def retrieveDataFromLines(self,lines):
        ...
        if 'Q-Chem' in lines[0]:
            start = [x for x in range(len(lines)) if 'TDDFT Excitation Energies' in lines[x]][0]
            end = [x for x in range(len(lines)) if 'SETman timing' in lines[x]][0]
            indEV = [x for x in range(start,end) if 'Excited state' in lines[x]]        # Find which lines have eV values
            indOS = [x for x in range(start,end) if 'Strength' in lines[x]]    # Find which lines have oscillator Strength
            # Create list of eV values from TDDFT/TDA result
            eV = []
            for i in range(len(indEV)):
                eV.append(float(re.findall('\d*\.?\d+',lines[int(indEV[i])])[1]))
            ...
            for i in range(len(nm)):
                OS.append(float(re.findall('\d*\.?\d+',lines[int(indOS[i])])[0]))
...
class DaltonViewApp:
    ...
    def pressedMakePlot(self):
        ...
        self.plotManager.plotTheoryData(self.ax,self.spectrum,self.sticks,self.checkvarPlotBars.get())
        self.canvas.draw()
    ...
    def pressedUpdatePlot(self):
        ...
        if self.ExperimentFilePath.get():
            self.plotManager.plotExperimentData(self.ExperimentFilePath.get().replace('"',''),self.scale_exp.get())
